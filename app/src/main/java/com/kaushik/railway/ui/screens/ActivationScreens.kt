            ...
            1 -> {
                ...
                RailCard {
                    ColumnBlock {
                        ...
                        CheckRow(
                            checked = terms,
                            text = "I accept the RailOne demo terms and confirm these details match my Govt. ID.",
                            onChange = { terms = it }
                        )
                        error?.let { Text(it, color = Color.Red, fontSize = 13.sp, modifier = Modifier.padding(top = 8.dp)) }
                        Spacer(Modifier.height(16.dp))
...
fun MpinScreen(vm: AppViewModel, onDone: () -> Unit) {
    ...
    var busy by remember { mutableStateOf(false) }
    FlowPage {
        StepHeading(1, 1, "Generate PIN", "4-digit MPIN and an optional biometric preference")
        RailCard {
            ColumnBlock {
                ...
                Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.fillMaxWidth()) {
                    Column(Modifier.weight(1f)) {
                        Text("Biometric authentication", color = Navy, fontWeight = FontWeight.SemiBold)
                        Text("Preference only. This demo does not enroll a fingerprint.", color = Color.Gray, fontSize = 12.sp)
                    }
                ...
                OrangeButton(if (busy) "PLEASE WAIT" else "SUBMIT", enabled = !busy) {
                    busy = true
                    error = null
                    vm.saveMpin(pin, confirm, biometric) { message ->
                        busy = false
        ...
        DemoNote("The PIN is stored as a hash on this device.")
    }
}
